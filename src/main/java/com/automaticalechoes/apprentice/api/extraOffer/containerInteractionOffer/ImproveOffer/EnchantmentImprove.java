...
public class EnchantmentImprove extends ImproveOffer{
    ...
    public static List<Enchantment> Enchantments(ItemStack itemStack,boolean isBook){
        if(isBook){
            return BuiltInRegistries.ENCHANTMENT.stream().filter(enchantment -> !enchantment.isCurse()).toList();
        }
        return BuiltInRegistries.ENCHANTMENT.stream().filter(enchantment -> enchantment.canEnchant(itemStack) && !enchantment.isCurse()).toList();
    }
