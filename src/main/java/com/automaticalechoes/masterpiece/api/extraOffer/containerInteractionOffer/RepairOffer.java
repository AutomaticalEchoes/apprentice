...
import com.automaticalechoes.masterpiece.api.TagKeyMap;
...
public class RepairOffer extends ContainerInteractionOffer<RepairOffer> implements LevelUpChangeOffer {
    ...
    public void appendHoverText(List<Component> p_41423_) {
        p_41423_ .add(Component.translatable("offer.cost_a").append(getCostA().getDisplayName()).append("x" + getCostA().getCount()).withStyle(ChatFormatting.DARK_RED));
        p_41423_.add(Component.translatable("offer.fix_type").append(Component.translatable(itemTag.location().toShortLanguageKey())).withStyle(ChatFormatting.DARK_GREEN));
        p_41423_.add(Component.translatable("offer.fix").append(String.valueOf(repairValuePerCost * getCostA().getCount())).withStyle(ChatFormatting.GREEN));
    }
